...
def run_pybullet(filepath):
    ...
    if args['action'] == 'Joint Velocity':
        ...
        ik_flag = True
    arg_dict = {'state_dim': args['state_dim'], 'action_dim': 4, 'max_action': args['max_action'], 'n': 5, 'discount': args['discount'], 'tau': 0.0005,
                'batch_size': args['batch_size'], 'epsilon':args['epsilon'], 'edecay': args['edecay'], 'ik_flag': ik_flag}
    ...
    manipulation = manipulation_phase_rl.ManipulationRL(
        hand, obj, x, y, state, action, reward, replay_buffer=replay_buffer, args=arg_dict, tbname=args['save_path'])
    # manipulation = manipulation_phase_rl.ManipulationRL(
