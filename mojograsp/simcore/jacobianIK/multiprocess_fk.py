...
class ForwardKinematicsSIM():
    ...
    def get_link_ids(self):
        ...
        for i in range(self.p.getNumJoints(self.hand_id)):
            j_info = self.p.getJointInfo(self.hand_id, i)
    ...
    def update_poses_from_sim(self):
        ...
        for id in self.link_ids:
            l_state = self.p.getLinkState(self.hand_id, id)
    ...
    def update_angles_from_sim(self):
        ...
        for i in range(len(self.current_angles)):
            self.current_angles[i] = self.p.getJointState(self.hand_id, self.link_ids[i])[0]
    ...
    def initialize_transforms(self):
        ...
        base_link_r = mh.create_rotation_matrix(self.p.getEulerFromQuaternion(self.current_poses[0][1])[2])
        ...
        self.current_angles.append(self.p.getJointState(self.hand_id, self.link_ids[0])[0])
        ...
        for i in range(1, len(self.current_poses)):
            ...
            mat_r = mh.create_rotation_matrix(self.p.getEulerFromQuaternion(self.current_poses[i][1])[2])
            ...
            self.current_angles.append(self.p.getJointState(self.hand_id, self.link_ids[i])[0])
    ...
    def debug_show_link_positions(self, points):
        ...
        if self.debug_id_new:
            self.p.removeUserDebugItem(self.debug_id_new)
        for i in points:
            i[2] = .05
        self.debug_id_new = self.p.addUserDebugPoints(
            points,
            [[255, 0, 0]] * len(points),
            pointSize=10)
